...
class AgentLoop:
    def __init__(
        ...
        self.llm = llm
        self.tools = tools
        # (生活事件的注入侧参数 2026-09-21 拆除:用户拍板"往事只走 recall 工具,
        #  不留常驻上下文"。生活事件现在**整条不进投影**,所以不再需要前缀 ——
        #  判定与范围见 core/session_log.derive_messages 的 docstring。
        #  ⚠️ `LIFE_EVENT_PREFIX` 那个模板仍留在内容层:它是**读侧**识别用的
        #  (剥掉被模型抄进正文的标记),日志里那 2 条脏字是唯一证据。)
    ...
    def run_turn(
        ...
        *,
        ...
        多会话(server):每次 run_turn 传目标会话的 log。实例服务所有会话,但
        **会话的东西全是参数** —— llm/tools 与内核循环共享,而**日志与该卡人格**
        (system_prompt,来自卡片快照)每轮由调用方传进来。
        (2026-09 每卡一套 life,DESIGN §12b:会话 = 角色卡,**各过各的日子**。
        早先这里写"她这个人跨会话一致"—— 那句只对**装配层**成立;人格层与生活层
        现在是每卡独立的,别再按"一个她跨会话一致"理解。)
