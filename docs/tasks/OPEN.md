...
| # | 事 | 用户原话 / 现状 | 怎么测 |
|---|---|---|---|
| T1 | **`CHAT_SITUATION` 尾巴那半句「你也有一些tool可用」要不要删** | ✅ **已测(2026-09-22 11:12,真模型 24 次调用)。结论:那半句测不出作用**,所以该不该删是**结构问题不是效果问题** —— 它是一句不跟随 registry 的能力声明,违反 `character/persona.py` 的铁律"能力唯一来源 = 本轮 schema + 工具用法段"。**建议删**;**但删它 = 改所有陪聊轮看到的 SYSTEM,是内容层文案、是你的字,我没有替你删** —— 说一声我删(只删那半句,「网名叫」那部分保留) | 仪器已留在仓库里:`test/subagent_prompt_lab.py` 新增 `A2`(与产品现状 `A0` **逐字相同,只去掉**「,你也有一些tool可用」)+ 变体 `SHIPPED-A2去半句`,其余(usage/desc/任务集)与 `SHIPPED-上线版` 完全一致 —— **这半句第一次被单独对照**(以前 A0 出现在 10 个变体 + SHIPPED 全部行)。**实测:6 个任务格委派率逐格相同,编造指纹也逐格相同**(两列都 `2/2 · 0/2 · 0/2 · 2/2 · 2/2 · 0/2`);⚠️ 2 次/格薄样本,按仓库自己的规矩**只能说"测不出作用",不能说"有害"**。复测:`py test/subagent_prompt_lab.py --variants SHIPPED --tasks t1,t2,t3,t4,s1,s3 --reps 4` |
...
| **内心面板 / 桌面工作区** | ⛔ **已裁决(2026-09-22 10:45):不留。** 用户原话:「你要分清楚,我发现了,你之前说的动过的部分其实都是你自己写文档的时候顺手的,**并不是我真的操刀过这一盘**,所以确实文档里有,但并非是我想让它有才有的,所以**项1 应该是不留**且**文档有擅自成分,得改**。」事实核对:`view.py` + 两个前端文件的首个 commit 都是 `2903b2a baseline`;**我上一轮拿 `8138640` 的"内心跟随当前卡"当"用户操刀过面板"是过度解读** —— 那条改的是**目标卡选择**(自走/补写打哪张卡,是真的),不是"要不要有面板"。**处置:按脉冲同款标注为不留/待砍(不摘);上一轮那两处 shadow 修改已撤回;文档擅自表述已改**(`STRUCTURE` §4/DESIGN/VISION/PROGRESS/LIFE_BACKFILL/ARCHITECTURE/TIMELINE 共 7 处加了带日期的更正批注) | 终局由用户定:**① 摘掉** = 删 `view.py` 的 workspace + life-events 两端点 + `index.html` 两个 pane + `app-presets.js` 的 `_refreshInnerLife()`(**预设 CRUD 要留**)/ ② 保留 = 那要用户**重新拍一次**"这是产品面"。**别人不要自己动** |
...
| **删光消息的卡还算"激活"吗** | ✅ **已裁决并落地(2026-09-22 10:45)**:用户口径 ——「如果是说当前会话 0 消息了的话,应该是整个对应日志归档,**会被补写等过滤掉**,当然**不继续补东西,就是死掉了**」。已把 `_has_user_talk` 的判据从**原始事件**改成**可见消息**(排 `shadowed_seqs()`)→ 消息被全删的卡自动退出 `life_target()` 与 `life_backfill_order()`。新增回归测试 `test/test_server_store.py` 的 `test_deleted_all_messages_kills_the_card_as_life_target`(反演验证:换回旧口径当场断言失败) | ⚠️ 一个**待你确认**的边界:我实现的是**判据**(看不见真人说过话 = 死),**没有**自动把日志搬进 `archive/`。自动归档是破坏性动作(会与 UI 当前会话状态打架),要的话一句话我加 |
