...
> **能放在锁外的只有「起这条线程」。**
> 从"读日志尾"开始 —— 那是**决策依据** —— 到"跑完 N 轮",必须一气呵成在
> 同一把锁里。依据和基于它的动作之间一旦松锁,依据就可能过期。
...
| | 做什么 | 代价 |
|---|---|---|
| **甲** | 就只补"最近激活那张";把 §12b 改成"其它卡的离线段**不补**" | 承认其它卡留空白(理由:你没跟它过) |
| **乙**(建议) | 启动时**遍历所有有历史的卡**,各自判断各自补 | 多张卡要补 → 开机等更久(N × 30–90s)。`_wake_decision` 已自带"无历史不补",纯空卡不会拖时间 |

**建议乙**,理由:它才对得上"每卡各过各的日子",也修掉用户观察到的
"启动时只有第一个会话触发补写"。⏳ **等你拍。**

> 注:甲/乙**都不需要用户消息当触发点**,都不需要动 `chat.py`。
> 若取乙,补写是串行的(同一把 `_lock`),所以 ③(`_backfill_clock` 全局)
> 仍然**不会被触发**,但它依旧是潜伏的地基问题。
