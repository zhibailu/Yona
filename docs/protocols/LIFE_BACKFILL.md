...
"一天难得有一两件值得记的事"。
交付形态:**UI 内心活动(life-events / workspace)看到一条连续的时间线**
(若干事件,各有起始时间),不是"开机补一句话"(那是假体验,已废弃)。

> ⛔ 【2026-09-22 10:45 更正】上面那句"**交付形态:UI 内心活动(life-events / workspace)…**"
> 把这两个面板当成交付面在写 —— 那层是**擅自加的**。本协议真正的交付物是**补出来的
> 那段时间线本身**(从事件日志投影),与有没有这块面板无关。**用户 2026-09-22 10:45
> 判定这两个面板不留**,2026-09-22 11:20 确认摘除 → 已删干净**。详见 `server/app/api/view.py` 的模块头。
...
start()(lifespan 调一次)
 └─ _maybe_backfill_life()                    # 只在这一处被调用
     └─ 后台线程 "yona-backfill" ─► life_backfill_order() 逐卡入队(下面每一项由 turn worker 持 _lock 跑 _card_job):
         ├─ order = _store.life_backfill_order()  # store.py 的 life_backfill_order():全部
         ...
         │   # 一张卡 = 一个队列项;_lock 由 turn worker 在 engine._turn_worker_loop
         │   # 的 with _lock: 拿,线程自己不抢
...
> `server/app/engine.py` 的 `_turn_worker_loop`(那句 `with _lock:`)拿,线程自己不抢;
> 启动时按 `life_backfill_order()`
> (`server/store.py` 的 `life_backfill_order()`:全部 `_has_user_talk` 的卡、`updated_at` 降序,**没有旗舰
> 兜底**)**逐卡** `_submit_turn(lambda s=sid: _card_job(s), priority=_QUEUE_SELF, sid=sid)`
> (`server/app/engine.py` 的 `_maybe_backfill_life` 里那个 `_run()`);
> `life_session_id()`
> (`server/app/engine.py` 的 `life_session_id()`)现在只服务自走/心跳/脉冲的目标卡。
...
> `PERSONA` / `CHAT_SITUATION` / `SELF_SITUATION`(三者都在 `character/personas.py`),没有任何 `BACKFILL_*`。
...
```
[引擎动态拼接的当轮数据 —— server/app/engine.py 的 _card_job 里那段 note 拼接,不是内容层常量]
f"这段时间(约 {_human_gap(e.budget_min*60)})里你只做了**一件事**——就是现在刚做完/正在做的这一件。{gap_note}"

gap_note(server/app/engine.py 的 _card_job 里的 gap_note):
  i == 0        → ""
...
> **引擎动态拼接的当轮数据**(`server/app/engine.py` 的 `_card_job` 里那段 `note` 拼接),不是内容层常量;真值 =
> `f"这段时间(约 {_human_gap(e.budget_min*60)})里你只做了**一件事**——就是现在刚做完/正在做的这一件。{gap_note}"`;
> `gap_note` 见 `server/app/engine.py` 的 `_card_job` 里的 `gap_note`(两种情况:空档 > 60s → 「距离你上一件事
> 做完已经过了约 X(中间的时间平平淡淡,没发生值得记的事)。」;否则 → 「你上一件事刚做完
> 不久。」);`character/personas.py` 的 `SELF_SITUATION` 只有一句。
...
> 不存在:`test/backfill_probe.py` 里 `situation=sm.BACKFILL_SITUATION` 那一处用 `sm.PERSONA` / `sm.BACKFILL_SITUATION`
> (`sm` = `server.app.engine`,见该文件第 17 行),而 `server/app/engine.py` 没有模块级
> `PERSONA`(只有 `personas_mod.PERSONA`,即 `server/app/engine.py` 里三个 composer
> 装配处那三句 `personas_mod.PERSONA`),
> `character/personas.py` 也没有 `BACKFILL_SITUATION`;`sm.SELF_SITUATION` 同样不存在。
> (`dist` / `inv` / `table` 三档不变。)
>
> 【2026-09-21 23:10 更正】`rate_curve_plot.py` 行把输出路径写错了 —— 真相:
> `test/rate_curve_plot.py` 的 `OUT = Path(__file__)… / "assets" / "design" / "rate_curve.png"` 那一行;
> 仓库里只有 `assets/design/rate_curve.png`,`test/rate_curve.png` 不存在
> (三张子图是真的,`fig, (ax1, ax2, ax3) = plt.subplots(` 那一段)。
...
>   > 一张卡一个队列项 | ✅ **已落**」互相打架 —— 真相:`server/store.py` 有
>   > `life_backfill_order()`、`server/app/engine.py` 的 `_maybe_backfill_life` 里那个
>   > `_run()` 已逐卡 `_submit_turn`,① **已落**。
...
| # | 缺口 | 现状(代码事实) |
|---|---|---|
| ① | 「其它卡等**下次被激活**再补」没实现 | ~~`_maybe_backfill_life()` 全项目只被 `start()` 调一次(`engine.py`),只检查 `life_session_id()` 那一张卡。~~ **① 已落(2026-09-17 取方案乙)**:`_maybe_backfill_life()` 仍只在 `start()` 被调一次(即 `server/app/engine.py` 的 `start()` 里那句 `_maybe_backfill_life()`),但内部按 `store.life_backfill_order()` 遍历**所有有历史的卡**。⚠️ 该承诺**在逻辑上就落不了地**(不是"没写"),见 §9.6 |
...
> (另一半「仍只被 `start()` 调一次」是对的)—— 真相:`server/app/engine.py` 的
> `_maybe_backfill_life` 里那个 `_run()`
> 遍历所有卡,顺序来自 `server/store.py` 的 `life_backfill_order()`;`start()` 那一处
> 调用在 `server/app/engine.py` 的 `start()` 里那句 `_maybe_backfill_life()`。
...
另外两个读点(~~`_now(log)`~~ `_epoch_now(log)` 与 `sys_by_source`)**手里都有 log**。

> 【2026-09-21 23:10 更正】`_now` 这个标识符在 `server/app/engine.py` 里全文件不存在 ——
> 真相:对应读点是 `_epoch_now(log)`(现在落在 `server/app/engine.py` 的 `_epoch_now(log)`)
> 与 `sys_by_source`(`server/app/engine.py` 的 `sys_by_source`)。
> 【2026-09-21 23:45 更正】上一条当时写的是 `_live_epoch(log)`(`server/app/engine.py:838-843`)
> —— 那个闭包**已被删**:2026-09 清理把它收进模块级 `_epoch_now(log)`,同一次收口还有
> `_is_backfill(log)` / `_values(...)`。
...
> 真相:`server/app/engine.py` 的 `user_turn_priority(sid)` 返回
> `_QUEUE_SELF if sid in _pending_backfill else _QUEUE_USER`,`server/app/api/chat.py`
> 的 `_run()` 里 `engine._submit_turn(..., priority=engine.user_turn_priority(sid))`
