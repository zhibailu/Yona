...
所以唯一自然的落点是:**那张卡即将跑聊天轮之前**。

**方案 A 把它变成一条直路**(B/C 已否决,不必再考虑别的组合):

```
chat.py _run():抢到 _lock
   ├─ ① 这张卡上次活跃到现在,够不够补写阈值?
   │     够 → 就地补完(**同一个锁内,事件排在你消息之前** ✓)
   ├─ ② 读该卡日志
   ├─ ③ run_turn(你这条消息)
   └─ ④ save_log
```

**① 与补写执行必须在同一把锁内,且在你这条消息落盘之前** —— 和 §9.3 同一条
约束:"读日志尾(决策依据)→ 补完"之间不能松锁。

⚠️ **仍待拍一处**:补写窗口的**终点**取"你这条消息的时刻"还是"此刻"?
前者语义更准(那段生活到你开口为止),后者更好写。
...
| `server/app/api/chat.py` | 加 ① 触发点(聊天轮之前、同一锁内) | ⏳ 未做 |
