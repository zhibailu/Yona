...
### 9.5 ⏳ 地基(#③):`_backfill_clock` —— 做 ① 之前必须先动它
...
**落地形态**(⏳ 未做):作息参数进**卡片快照**(与人格覆盖同一处 `meta.json`),
缺省 = 旗舰那套(现 `params.py` 的值);`_life_gate` / `_heartbeat` 从
"全局一个"变"每卡一份"。

> ⚠️ **与 §12b 现有拍板冲突** —— §12b 写的是"全局单套闸门(3 次/天),**不做每卡
> 独立节奏**"。本条是 2026-09-17 的新判断;**落地前必须在 DESIGN §12b 同步修订**,
> 别让两处并存。
...
| 文件 | A 下的动作 | 状态 |
|---|---|---|
| `server/app/engine.py`(`_maybe_backfill_life`) | 整段纳入 `_lock`;删锁外 sleep | ✅ **已落** |
| `server/params.py` | 删 `BACKFILL_START_DELAY_SEC`(定义 + `_ROWS` 行) | ✅ **已落** |
| `server/app/engine.py`(`_backfill_clock`) | 参数化 —— ③ 的地基 | ⏳ 做 ① 之前必须先动 |
| `server/app/api/chat.py` | **不动** —— 补写不由用户消息触发(见 §9.6) | — |
