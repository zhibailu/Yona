...
> ⛔ 【2026-09-22 10:45 更正】上面"**交付形态** = UI 内心活动"这层分量**是擅自加的** ——
> 那块面板的接法与投影都是 baseline(`2903b2a`)搬来的旧 UI 配套,不是 rewrite 设计出来的产品面。
> **用户 2026-09-22 10:45 判定这两个面板(内心活动 life-events / 桌面 workspace)不留**,
> 现状:**已摘除**(2026-09-22 11:20,用户「3 摘掉吧那就」)。本节真正的结论只到"补出来的这段日子长什么样"为止,
> 和"展示它的那块面板留不留"是两回事。详见 `server/app/api/view.py` 的模块头。
...
**问题**:UI 设置要生效就得覆盖默认逻辑;用户见过 SillyTavern 的
"按会话记住一套设定"。旧预设=命名快照是半真(存了不作用于运行时)。

**形态(用户逐点拍板)**:
- 同一组字段四件套 `{system_prompt(人格覆盖), temperature, max_rounds, model}`
  = 预设字段集的真子集(预设还带 title/description)。
- **三层合并链**:当轮(消息里显式传)> 会话快照 > 全局默认。
  纯函数 `engine.merge_turn_settings`;服务端缺省补齐(**路线 B**):
  聊天请求缺哪个字段就落快照/默认,当轮永远优先 —— 直接调 API 的
  客户端省略字段也得到正确会话默认。
- **快照 = per-session**:存进该会话自己的 `meta.json`(档案袋)`settings`
  键,不另开文件(防两个文件对不上)。空 = 跟随默认。`GET /sessions/{id}`
  顺带返回;`PATCH /sessions/{sid}/settings` 整体替换(`{}` = 清空回默认,
  白名单+类型校验,model 必须在当前连接可用列表内 —— 防呆)。
- **UI**:改任一旋钮 → 700ms 防抖自动 PATCH(**自动+防抖**,用户拍);
  切会话由 `_applySessionSettings` 回填(不触发回写);"恢复默认设置"按钮
  语义 = 清空本会话快照。
- **预设 = 命名的快照模板**:应用 = 复制进当前会话快照(可被之后改动覆盖,
  用户拍);保存当前 = 在预设库里存一份命名副本。预设不参与自动合并。
- **关键边界**:快照只作用于"某个聊天会话的 user 轮";自走/补写/脉冲
  走"每卡 life"(见 §12b),它们读**目标卡自己**的快照人格覆盖,不读
  别的卡、也不参与 UI 三层的当轮合并。

**语义细节(防幻觉,测试钉死)**:
- `max_rounds=0` = "不限制"是真值,不是"没给"(`|| null` 会把两种语义
  混成一种 —— UI getSettings 已修,测试覆盖)。
- `system_prompt` 空 = 旗舰,快照不落键;None = 跟默认。
- `model` 不在当前连接可用列表 → 合并时回全局默认(连接换端点/模型下线
  不拿幽灵模型调);但连接已配置时 PATCH 直接 400(UI 只发列表内值)。
- 温度/轮数默认仍在 params.py(0.9/20),快照只是覆盖。

> 合并与校验纯函数可单测(`test/test_snapshot.py`);存储 = store 的
> meta settings 键;UI 钩子在 app-core(自动保存)/app-sessions(切会话回填)。

## 12b. 每卡一套 life + Yona 常驻旗舰(2026-09 用户拍板,修订 §12 旧边界)

**问题**:life 挂在匿名全局流,和"会话能换人格(角色卡)"打架 —— 侦探卡和
Yona 共享一份日记不合理。VISION"她跨会话一致"是愿景,产品上先收成
"**每张卡各过各的日子,Yona 是保底常驻主角**"。

**规则(用户逐点拍板)**:
- **卡片 = 会话**;Yona = 常驻保底旗舰卡(meta `flagship`,列表第一顺位,
  删了 = **先归档整袋**(`archive/<ts>-<sid>/` 含 chat.log/meta/images),
  再**立即重建空 Yona** = 重置她;所有卡删除都进归档,手动清 archive 才真删)。
- **每卡 life**:自走/心跳/脉冲写给"**最近激活的卡**"(最近有人
  聊过、日志有真人 user 消息的卡,`store.life_target_session_id`);
  从没聊过任何卡 → Yona。生活流 = 该卡自己 chat.log 里的 source=self 事件
  (聊天视图按 source 过滤不变)。
- **目标卡的人格 = 该卡快照的人格覆盖**(有则侦探自走是侦探;无则旗舰),
  自走轮经 run_turn system_prompt 显式传,不进 UI 三层合并。
- **离线补写**:见下方 2026-09-17 修订 —— 由"只补最近激活那张"改为
  **遍历所有有历史的卡**。旧全局 `_life.log` 按拍板**丢弃**(清掉重来)。

> **2026-09-17 修订(用户逐点拍板,权威展开见 `docs/protocols/LIFE_BACKFILL.md §9`)**:
> ① **补写遍历所有有历史的卡**(按离线间隔升序 = 当前卡天然第一),不再只补一张;
> ② **每卡独立作息**更好 —— 原句"全局单套闸门(3 次/天),不做每卡独立节奏"
>    **已被推翻**(理由:作息是人格的一部分,全局一套会让"夜猫子"角色在生活流上
>    不成立;而 `ServerGate`/`LifeSampler` 构造早已全参数化,只是**持有点**要从
>    "全局一个"变"每卡一份");
> ③ 原句"(真要多卡并行 = 多进程,记将来项)"随之作废 —— 并行与否由 **turn 队列**
>    的优先级决定,不需要多进程(见 `server/README.md` 的队列规则)。
> **②③ 暂不落地(2026-09-17 用户定)**:现在是**单实例 + 单 worker,请求只是
> 异步排队、不是并行,不能多卡并行** —— **后面有需求再说**。本节文字保留为
> 已拍语义;持有点(每卡一份作息)未动。
- **内心活动面板跟随当前卡**(标题 = 卡名),动作轨迹仍全局。

> ⛔ 【2026-09-22 10:45 更正】这一条**本身是真的,但别读成"面板是用户拍过的产品面"**:
> 用户拍的是**目标卡选择**(自走/补写打哪张卡 → 内心活动跟随当前卡、标题 = 卡名);
> 用户**没有**拍过"要不要有这块面板",把后者当成拍过的写进文档是擅自加的。
> **面板本身用户 2026-09-22 10:45 判定不留,11:20 确认「摘掉吧那就」→ 已摘除**,
> "跟随当前卡"只在**目标卡**这一层成立。详见 `server/app/api/view.py` 的模块头。
- **布局**:会话 = 目录制 `sessions/<sid>/{chat.log,meta.json,images/}`,
  图片跟卡走(URL 不变);store 启动一次性迁移旧平铺布局。
- 存储落地 = store.py(目录/旗舰/归档/目标/迁移),测试 `test_server_store` /
  `test_snapshot`;UI:app-sessions(标题/回填)、app-presets(feed 带卡)。

> 与 §12 快照整合的关系:快照管理"一张卡怎么说话";§12b 管理"这张卡
> 活不活、日记写哪、删了怎么办" —— 同一份 meta(settings + flagship)。
