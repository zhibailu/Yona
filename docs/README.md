...
5. **引用任何源码取值(枚举 / 字段名 / 常量 / 函数签名 / 行号)前先 grep 到定义**;
   ⛔ **但不要再往文档里写行号** —— 2026-09-22 彻查发现:一次代码清理就把全仓 **约 250 处**
   `文件.py:行号` 指针顶偏(被引最多的那处从 `:120` 漂到 `:193`)。
   **正确写法**:引**符号名**(`make_usage_section()` / `_is_backfill()` / `Tool.retain_result`),
   或引**一句可 grep 的原文**;只在"同文件同名符号出现多次、必须靠行号区分"时才写行号,
   并注明"行号会漂,以符号为准"。
6. 单个文件超过约 800 行就该拆;一个目录超过约 8 个文件说明分类粒度不对。
7. **写"已落地 / 已接线 / 已生效"必须先找到调用点**(不是定义)—— 见 skill 的 §三之二;
   定义存在 ≠ 生效(参数带默认值、调用处不传 = 功能没开)。
8. **行数 / 端点数 / 测试数一律以代码为准**,不要在文档里写死;非写不可就加一句
   "实测于 <日期>,会漂"。
