...
- **[时间线] 只挂独处轮,不挂陪聊轮 ✅(2026-09 用户指出修正)**:陪聊轮正在
  ...
  说话多久)。
- **事件轮当前时间 = 事件 start(自走/心跳/脉冲/强制同款)✅(2026-09 用户
  拍板修正)**:此前命中事件的自走轮把 [当前时间] 停在触发/注入时刻(如 +4h
  后显示 13:01、时间线整段"4 小时前"),start 抽完即弃 —— 模型看到的是
  "现在 13:01 + 距上次 4 小时 + 预算 34 分"这种对不上的视图。修正:一轮 =
  一个事件,像补写一样锚到事件起点叙述 —— `begin_self_wake` 新增
  `_wake_anchor["start"]` 记事件 start,触发方(引擎 LifeLoop/脉冲走
  `_anchor_event_view`:世界钟=_backfill_clock=start、游标=start+预算;lab
  同款)把视图拨到 start:[当前时间]=start、[时间线] 从 start 派生(不再是
  整段空窗)、[时间预算]=该事件预算(强制轮两个随机数 = start 与预算,都会
  落在视图里)。自语落事件结束时刻(start+预算)→ 日志尾自动前移,下一轮
  可消费区间从事件结束起 = 拍板锚推进。
