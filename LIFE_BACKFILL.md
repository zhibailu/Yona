...
> **协议权威**:DESIGN.md §10(用户拍板版)。本文件是它的完整展开:
> 算法、数学性质、架构、内核扩展、内容视角、参数现状、代码地图、
> 测试与验证数据、废弃方向、剩余开放项。两者应保持一致:改协议先改
> DESIGN.md §10,再同步本文件。进度条目见 MAP.md #22。

> **2026-09 修订(每卡 life,用户拍板)**:生活流不再落匿名全局 `_life`
...
> 卡本人,否则旗舰)。补写锚点 = 目标卡日志尾部。完整规则见 DESIGN.md §12b。
...
`_life` 生活日志,像朋友圈/日记 —— "一天难得有一两件值得记的事"。
...
补写轮 = **普通自走轮**:`_loop.run_turn(source="self", log=_life,
...
| 事件时间戳 | 墙钟 | 落历史时刻 |
| persona | SELF / CHAT(实时视图) | BACKFILL(补写视图) |
...
### 5.1 BACKFILL_PERSONA(server/main.py)

当下视角:"此刻大约是上方的 [当前时间],你一个人,刚做完或正在做一件
具体的事。把这件事说出来……像此刻随手记一笔、像发一条给在意的人看的
状态。就写当下这一件,有头有尾,平静、具体、有生活气,两三句话以内。"

**禁令**(用户点名,写死在 persona):
- 不回溯别的时间段("今天下午我…"这类不写)
- 不自报时间(时间由系统给)
- 不写情绪独白 / 不自怨自艾(点名禁"眼泪掉进碗里")
- 不要任务汇报腔 / 不提"工具/系统/模型"
