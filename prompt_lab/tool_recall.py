...
⚠ 换个仓库跑之前先看这段 —— 台子读的是**某个人的真聊天记录**,而聊天记录
  ...
  ────────────────────────────────────────────────────────────────
  ① 卡片      test/recall_probe.py:47-48
              `SID` / `LOG_PATH` → 指到 data/sessions/<你的卡>/chat.log
  ...
  (test/recall_probe.py:443),纯陪聊只会产生 `talk` —— 那样 life 全空,
  ...
  占位 bigram 并**说清用的是哪个**(test/recall_probe.py:551)。
...
⚠ 曾经不成立、现已成立的一条(2026-09-19 记,2026-09-20 已修)
  ...
  当时做不到:`fold_tool_traces` 关着(engine.py:778 一带),`retain_result=False`
  那行声明**空转**,第一轮 recall 的原文还留在第二轮上下文里。
  现在 `fold_tool_traces=True`(engine.py:787),第二轮 roles =
...
#   取值与 test/recall_router_probe.py:59、test/recall_stage1_lab.py:55 相同。
...
# ============================================================
#
# 台子读的是**真卡片**(test/recall_probe.py:47-48 写死的那张),而台子是拿来
# 反复乱问的。真卡片被实验轮改掉一个字,后面所有测量用的"事实底座"就变了 ——
# 那是**仪器**坏掉,不是产品坏掉,而且事后从台面上看不出来。
#
# 现状实测(2026-09-20,跑真轮前后对 data/ 下 10 个文件取 MD5):**逐字节一致**。
# 原因是写盘只发生在 eng._store.save_log
# (engine.py:554 / :993、api/chat.py:131、main.py:280),而台子直接调
# eng._loop.run_turn,**不经过它们**;子运行的 SubRunStore 也是 `store=None`
# (core/subrun.py:283 只在给了仓时才落 jsonl);检索库是 sqlite `:memory:`
# (test/recall_probe.py:641)。
#
...
def resolve_variant(name: str) -> str | None:
    """按名字找变体,允许只写前缀(容错逻辑在 recall_probe,单一来源)。"""
    return P.pick_variant(name)


def variant_of(key: str) -> tuple[str, str, str]:
    """→ (description, usage, scope 参数说明) —— 后两项是两条不同的通道。"""
    return P.variant_parts(key)
...
class Apparatus:
    ...
    接线顺序有讲究,**不能动**:
      ...
      ① `eng._tools.register(recall)` 必须在 `_build_engine` **之前**。
         AgentLoop 只在**构造时**快照一遍 retain_result(core/loop.py:84-87),
         晚注册的工具痕迹在后轮会被折掉,血缘就从视图里消失了。
         这也是 engine.py:86-88 那条"工具在模块加载时注册一次"约束的同一条。
    ...
    def __init__(self, variant: str, *, memory: bool = True,
        ...
        # 系统那只钟当场拨到与工具同一个刻度(engine.py:695-706 的 _clock_override
        # 就是产品里"现在几点"的唯一来源,普通轮/世界段/时间线全读它)。
        # **log 的时间游标必须一起拨** —— 真人消息进上下文时带的 [HH:MM] 戳走的是
        # 日志事件自己的时间(core/session_log.py:385),只拨 _clock_override 的话
        ...
        # 与 recall_e2e.py:205 同一组参数(不设 min_score:阈值当相关性判据已被
        # 实测证伪,见 recall_probe.py:562-565,只留工具内部那道地板)。
        ...
        # ① 先注册工具,再建引擎(见类文档)
        if "recall" not in eng._tools.names():
        ...
        #    理由与 recall_e2e.py:212 同:工具竞争是阶段一另外测过的变量,
    ...
    def sections(self, source: str = "user") -> list[tuple[str, str]]:
        ...
        为什么不直接调 eng.system_component_sections:它写死读 eng._tools
        (engine.py:820)。本轮传了工具子集时,它会把"没开放的工具"也念出来,
    ...
    def set_variant(self, name: str) -> str:
        key = resolve_variant(name) or "★定稿"
        desc, usage, scope_desc = variant_of(key)
        self.tool.description = desc
        self.tool.usage = usage          # 出厂不带 usage,由装配处贴(与 engine 一致)
    ...
    def show_tool_prompts(self) -> None:
        ...
        两件东西分属两条通道(core/tools.py:20-24):
        ...
        # (core/subrun.py: 不带人格,SYSTEM 由内容层给,engine.py:155 拼)。
...
def _console_cb(log, shown: list[int], show_tools: bool = True):
    ...
    同一步里她可以**一次声明好几个**调用(引擎并发跑,见 core/loop.py:393-414:
    先落 N 条 tool/call,再落 N 条 tool/result,两边都带同一个 call id)。
...
def _print_detail(app: Apparatus, user_msg: str) -> None:
    """细节视图:每次调用的messages 输入 + 工具 SQL + 命中原文。"""
...
def _pick_variant(app: Apparatus) -> None:
    ...
    if not raw:
        return
    name = keys[int(raw) - 1] if raw.isdigit() and 1 <= int(raw) <= len(keys) else raw
