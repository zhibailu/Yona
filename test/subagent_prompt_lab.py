...
# A2 = **产品删掉那半句之后的样子**(2026-09-22 临时任务栏 T1 的结论)。
# 起因:用户在临时任务栏里要求测「CHAT_SITUATION 尾巴那半句『你也有一些tool可用』
# 要不要删」。而这半句**从来没被单独测过** —— A0 出现在 10 个变体 +
# `SHIPPED-上线版` **全部**行里,唯一的 A1 也照样带着它,所以没有"有它/没它"的对照。
#
# 测出来的结果(2026-09-22 11:12,真模型 24 次):**两列逐格相同**
# (委派率 + 造指纹都是 `2/2 · 0/2 · 0/2 · 2/2 · 2/2 · 0/2`)→ 那半句**测不出作用**。
# → 用户据此拍板**删掉**(结构理由:它是不跟随 registry 的能力声明,违反
#   `character/persona.py` 的铁律"能力唯一来源 = 本轮 schema + 用法工具段")。
#   `personas.CHAT_SITUATION` 现在**已经没有那半句**,所以上行的 `A0` 与 `A2` 目前
#   **是同一个串**,这个变体暂时没有差异 —— **故意留着**,理由两条:
#     · 它是"已经删掉的那半句"的**存档**:将来要复测"加回来会不会更好",
#       把下面 `A2` 换回带半句的串即可,变体表不用动;
#     · 它记着这次实验的做法(只改一处、其余全同),下次测情境段别的半句照抄这个形状。
# 顺带说明 **薄样本纪律**:2 次/格只能说"测不出作用",不能说"有害" ——
# 要更强结论请 `--reps 4` 以上再跑一次。
#
# 复测反向时,把下一行换成注释里那句(带半句的),别的什么都不用动:
